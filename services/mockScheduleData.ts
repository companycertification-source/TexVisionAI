 ...
 * 
 * All data is lazy-loaded via getter functions to prevent
 * "Cannot access before initialization" errors in production builds.
 */

import { WorkStation, InspectionSchedule, ScheduledInspection } from '../types';

// ==================== LAZY LOADING PATTERN ====================
// All mock data is generated on first access to prevent module initialization issues

let _workStations: WorkStation[] | null = null;
let _inspectionSchedules: InspectionSchedule[] | null = null;
let _scheduledInspections: ScheduledInspection[] | null = null;

// Work Stations Generator
function createMockWorkStations(): WorkStation[] {
    return [
        {
            id: 'ws-001',
            code: 'CUT-01',
...
// Inspection Schedules Generator
function createMockInspectionSchedules(): InspectionSchedule[] {
    const workStations = getMockWorkStations();
    return [
        {
            id: 'sch-001',
        ...
        },
        {
            ...
            interval_minutes: 120,
            is_active: true,
            work_station: workStations[3]
        }
    ];
}

// Scheduled Inspections Generator
function createMockScheduledInspections(): ScheduledInspection[] {
    const schedules = getMockInspectionSchedules();
...
export function getMockWorkStations(): WorkStation[] {
    if (!_workStations) {
        _workStations = createMockWorkStations();
    }
    return _workStations;
}

export function getMockInspectionSchedules(): InspectionSchedule[] {
    if (!_inspectionSchedules) {
        _inspectionSchedules = createMockInspectionSchedules();
    }
    return _inspectionSchedules;
}

export function getMockScheduledInspections(): ScheduledInspection[] {
    if (!_scheduledInspections) {
        _scheduledInspections = createMockScheduledInspections();
    }
    return _scheduledInspections;
}

// ==================== BACKWARD COMPATIBILITY ====================
// These exports are deprecated - use the getter functions above
// They are kept empty arrays to prevent import errors
